...
export function MovieDetail({ movie }: MovieDetailProps) {
    ...
    const [isFavorited, setIsFavorited] = useState<boolean>(() => storeIsFavorite(String(movie.id)));

    useEffect(() => {
        const unsub = subscribeFavorites((ids) => setIsFavorited(ids.includes(String(movie.id))));
        return unsub;
    }, [movie.id]);
    ...
    const image = (movie as any).imageUrl ?? (movie as any).thumbnailUrl ?? "";
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="aspect-video w-full bg-black rounded-xl overflow-hidden shadow-2xl border border-zinc-800">
                        <iframe
                            width="100%"
                            height="100%"
                            src={(movie as any).trailerUrl ? (movie as any).trailerUrl + "?autoplay=1&mute=1" : ""}
                            title="Movie Trailer"
                            frameBorder="0"
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-3 text-sm font-medium text-gray-300 border-b border-zinc-800 pb-4">
                            <span className="bg-zinc-800 px-3 py-1 rounded-full text-white">{(movie as any).year ?? movie.year}</span>
                            <span className="bg-zinc-800 px-3 py-1 rounded-full text-white">{movie.duration}</span>
                            <span className="bg-zinc-800 px-3 py-1 rounded-full text-white">{(movie as any).genre ?? (movie as any).category}</span>
                        </div>
                                        ...
                                        <div className="px-4 pb-4 animate-in slide-in-from-top-2 duration-300 space-y-4">
                                            ...
                                            <div className="space-y-2 pt-2">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                    {((movie as any).platforms ?? []).map((platform: any) => (
                                                        <a
                                                            key={platform.name}
                                                            href={platform.url}
                ...
                <div className="flex flex-col gap-6">
                    <div className="relative aspect-[2/3] w-full max-w-sm mx-auto lg:max-w-none rounded-xl overflow-hidden shadow-2xl border border-zinc-800">
                        <img src={image} alt={movie.title} className="w-full h-full object-cover" />
                    </div>
                    ...
                    <div className="flex gap-3">
                        ...
                        <Button
                            ...
                            className={`flex-1 gap-2 border-zinc-800 bg-zinc-900/50 hover:bg-zinc-800 transition-colors ${isFavorited ? 'text-yellow-500 border-yellow-500/50' : 'text-white'}`}
                            aria-pressed={isFavorited}
                            aria-label={isFavorited ? "Usuń z ulubionych" : "Dodaj do ulubionych"}
                        >
                            <BookmarkPlus className={`w-5 h-5 ${isFavorited ? 'fill-current' : ''}`} /> {isFavorited ? 'W ulubionych' : 'Ulubione'}
                        </Button>
                    </div>

                    <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800 flex items-center justify-center gap-3">
                        <Eye className="w-5 h-5 text-blue-400" /> <span className="font-semibold text-lg">{(movie as any).views?.toLocaleString?.() ?? movie.views?.toLocaleString?.() ?? 0}</span> <span className="text-gray-400 text-sm">osób obejrzało</span>
                    </div>
